...
/**
 * An activity representing a single ChildItem detail screen. This
 * activity is only used on narrow width devices. On tablet-size devices,
 * item details are presented side-by-side with a list of items
 * in a {@link ChildListActivity}.
 */
public class ChildDetailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            ...
            arguments.putLong(ChildDetailFragment.ARG_ITEM_ID, getIntent().getLongExtra(ChildDetailFragment.ARG_ITEM_ID, 0));
            ChildDetailFragment fragment = new ChildDetailFragment();
            fragment.setArguments(arguments);
            getSupportFragmentManager().beginTransaction()
                    .add(R.id.childitem_detail_container, fragment)
                    .commit();
        }
    }
